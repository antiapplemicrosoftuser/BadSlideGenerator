...
def generate_slide():
    ...
    # グラデーションの方向を直線の傾きに基づいて設定
    for x in range(width):
        for y in range(height):
            # 傾きを考慮したグラデーションの計算
            t = (x * slope + y) / (width * slope + height)
            t = max(0, min(1, t))
    ...
    for width, height, position in zip(graph_widths, graph_heights, graph_positions):
        ...
        graph_positions_list.append((position + width / 2, bottom_pos if bottom_pos <= 300 else bottom_pos - height / 2))
    ...
    for _ in range(15):  # 15回文章を表示する
        ...
        # テキストの位置を保存
        text_positions.append((x_position + text_width / 2, y_position + text_height / 2))
        
        # 次の文章のために y_position を更新
        y_position += text_height + random.randint(10, 30)
    
    # 文字やグラフをランダムに矢印で繋ぐ
    all_positions = text_positions + graph_positions_list
    num_arrows = random.randint(5, len(all_positions) - 1)
    for _ in range(num_arrows):
        start_pos = random.choice(all_positions)
        end_pos = random.choice(all_positions)
        if start_pos != end_pos:
            arrow_width = random.randint(1, 5)
            draw.line([start_pos, end_pos], fill="black", width=arrow_width)
            
            # 矢印の先端を描画
            arrow_size = arrow_width * 2  # 矢印の太さに応じて先端の大きさを設定
...
# 保存
fileName = f"generateImages/Ver46/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver46.png"
