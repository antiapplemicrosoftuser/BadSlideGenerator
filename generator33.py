...
def generate_slide():
    ...
    # 画面を2つの領域に分け、それぞれに背景色を適用
    top_rect = (0, 0, width, border)
    bottom_rect = (0, border, width, height)
    draw.rectangle(top_rect, fill=bg_color1)
    draw.rectangle(bottom_rect, fill=bg_color2)
...
### 自力
fileName = f"generateImages/Ver33/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver33.png"
