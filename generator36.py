...
def plot_graph(draw, x_values, y_values, bar_width, bottom_pos):
    ...
    if graph_type == 'line':
        ...
        # 直線の終点を計算
        end_x = start_x + width * np.cos(angle)
        end_y = start_y + width * np.sin(angle)
        draw.line([(start_x, start_y), (end_x, end_y)], fill=(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)), width=random.randint(1, 5))
    elif graph_type == 'bar':
...
### 自力
fileName = f"generateImages/Ver36/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver36.png"
