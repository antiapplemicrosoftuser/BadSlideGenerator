...
def plot_graph(draw, x_values, y_values, bar_width):
    ...
    if graph_type == 'line':
        ...
        for y_tick in range(0, 600, 100):
            draw.line([(x_values[0], y_tick), (x_values[0] - 5, y_tick)], fill="black", width=2)
...
### 自力
fileName = f"generateImages/Ver25/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver25.png"
