...
def generate_slide():
    ...
    for width, height, position in zip(graph_widths, graph_heights, graph_positions):
        ...
        # 長方形でグラフを囲む
        draw.rectangle([(position, 0), (position + width, height)], outline="black", width=2)
...
### 自力
fileName = f"generateImages/Ver28/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver28.png"
