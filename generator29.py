...
def generate_slide():
    ...
    for width, height, position in zip(graph_widths, graph_heights, graph_positions):
        ...
        # 長方形でグラフを囲む
        if (bottom_pos <= 300):
            draw.rectangle([(position - bar_width, bottom_pos), (position + width + bar_width, height + bottom_pos)], outline="black", fill="grey", width=2)
        else:
            draw.rectangle([(position - bar_width, bottom_pos - height), (position + width + bar_width, bottom_pos)], outline="black", fill="grey", width=2)
...
### 自力
fileName = f"generateImages/Ver29/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver29.png"
