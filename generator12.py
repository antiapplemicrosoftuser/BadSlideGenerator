...
def plot_random_graph(x_values, y_values, draw):
    ...
    if graph_type == 'line':
        ...
        for x, y in zip(x_values, y_values):
            draw.rectangle([(x - bar_width // 2, y), (x + bar_width // 2, y + random.randint(10, 50))], fill=(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)))
...
### 自力
fileName = f"generateImages/Ver12/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver12.png"
