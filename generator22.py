...
def plot_graph(draw, x_values, y_values, graph_type, bar_width):
    if graph_type == 'line':
        ...
        for x, y in zip(x_values, y_values):
            draw.rectangle([(x - bar_width / 2, y), (x + bar_width / 2, 600)], fill=(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)))
...
### 自力
fileName = f"generateImages/Ver22/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver22.png"
