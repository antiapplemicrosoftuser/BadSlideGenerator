...
# グラフの種類
def plot_graph(draw, x_values, y_values, graph_type):
    if graph_type == 'line':
        ...
        for x, y in zip(x_values, y_values):
            draw.rectangle([(x - 5, 0), (x + 5, y)], fill=(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)))
...
def generate_slide():
    ...
    for width, height, position, graph_type in zip(graph_widths, graph_heights, graph_positions, graph_types):
        ...
        y_values = np.random.randint(50, max(height - 50, 50), 10)
        plot_graph(draw, x_values, y_values, graph_type)
...
### 自力
fileName = f"generateImages/Ver16/" + str(datetime.datetime.now()).replace(' ', '_').replace(':', '-').replace('.', '-') + "_ver16.png"
